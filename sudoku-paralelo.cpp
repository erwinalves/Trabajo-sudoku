...
typedef struct Nodo{
    int numero;
    bool esFijo; //primero para comprobar si es ingresado por la consola, luego comprobar si es unico.
    Nodo* link;
};
typedef Nodo* Lista;
...
void llenarListasMatriz(int valor, int aux[][9]){
    for(int i=0;i<9;i++){
        for(int j=0;j<9;j++){
...
int main(int argc, char* argv[]){
    ...
    while(j<largoDetalle){
        if(matriz[detalle[j]][detalle[j+1]]->numero==0){
            ...
            contador[detalle[j+2]-1]++;//va sumando en vector contador, para saber cuantos datos de cada numero hay en la lista
            j=j+3;
        }
        else{
    ...
    for(int i=0;i<9;i++){
        ...
        llenarListasMatriz(indice+1,matrizAux);//se suma uno por que los valores que entrega indice parten desde el 0
    }
